...
class API extends CI_Controller {
	...
	public function relation(){
		...
		///$user = 'hvezaskar@gmail.com';
		
		//$group = $this->Modul_Model->read("select `group` from person where `key` = '".$user."' and `index` = 0 ")[0]["group"];
		
		$person = $this->Modul_Model->read("select p.*, 
					(select count(*) from person pp where pp.`parent` = p.`key`) as 'childCount', 
					(select count(*) from person pp where pp.`parent` = p.`parent` and pp.`key` != p.`key`) as 'siblingCount' 
					from person p where p.`group` = '".$group."' order by p.`index`");
	...
	public function getRelative(){
		$user = $this->input->post("user");
		//$group 	= $this->input->post("masterGroup");
		//$user = 'usi@gmail.com';
		...
		if($this->input->post("target")){
			...
			$user	= $this->Modul_Model->read("select u.*, ifnull((select `status` from `relative` where `relative` = u.email and `user` = '".$user."' and `status` <> 40 limit 0, 1 ), -1) as 'relative_status',
						...
						(select `group` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'group' 
			...
			$user	= $this->Modul_Model->read("select u.*, ifnull((select `status` from `relative` where `relative` = u.email and `user` = '".$user."' and `status` <> 40 limit 0, 1 ), -1) as 'relative_status',
						(select `key` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'master',
						(select `group` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'group' 
						from `user` u where u.email in (select `relative` from relative where `user`  = '".$user."' and `status` <> 40 ) order by tanggal_lahir ");
		}
	...
	public function getPersonDetail(){
		...
		//$target = "uqhy@gmail.com_6395711";
		//$target = "uqhy@gmail.com_9837295";
		
		/* Check Couple */
		$masterCouple = []; $isCouple = false;
		
		$group = $this->input->post("group");
		
		$group2		= $this->Modul_Model->read("select `group` from person where `key` = '".$user."' and `index` = 0 ")[0]["group"];
		$groupMatch	= $this->Modul_Model->read("select `group` from person where `key` = '".$target."' ");
		
		$isGroupMatchExist = false;
		for($i = 0; $i < count($groupMatch); $i++){
			if($groupMatch[$i]["group"] == $group2){
				$isGroupMatchExist = true;
			}
		}
		if($isGroupMatchExist){
			$group = $group2;
		}
		
		
		$masterCouple 	= $this->Modul_Model->read("select `key` from person where `couple` = '".$target."' and `group` = '".$group."' ");
		
		//echo json_encode($masterCouple);
		if(count($masterCouple) > 0){
			$isCouple = true;
			$masterCouple = $masterCouple[0]["key"];
		}
		
		$myself		= $this->Modul_Model->read("select *, 'me_died' as 'of' from `user` where `email` = '".$target."'");
		if(!$isCouple){
			$parentKey	= $this->Modul_Model->read("select `parent` from person where `key` = '".$target."' and `group` = '".$group."' ");
			if(count($parentKey) > 0){
				$parentKey = $parentKey[0]["parent"];
			}
			else{
				$parentKey = "";
			}
			//$group		= $this->Modul_Model->read("select `group` from person where `key` = '".$target."' and `group` = '".$masterGroup."' ")[0]["group"];
			$child		= $this->Modul_Model->read("select * from `user` where first_name <> '' and first_name <> '*' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$target."') order by tanggal_lahir ");
		}
		else{
			$parentKey = "";
			//$group		= $this->Modul_Model->read("select `group` from person where `key` = '".$masterCouple."' and `group` = '".$masterGroup."' ")[0]["group"];
			$child		= $this->Modul_Model->read("select * from `user` where first_name <> '' and first_name <> '*' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$masterCouple."') order by tanggal_lahir ");
		}
		...
		if($parent != "-"){
			if(!$isCouple){
				...
				$mother		= $this->Modul_Model->read("select * from `user` where email = (select couple from person where `key` = '".$parentKey."' and `group` = '".$group."'  limit 0, 1) ");	
				...
				$couple		= $this->Modul_Model->read("select * from `user` where email = '".$masterCouple."' ");	
			}
			
			if(!$isCouple){
				$q = "	select *, tanggal_lahir as 'tanggal', 'child' as 'of' from `user` 
						where email <> '".$target."' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$target."')
						union
						select *, tanggal_lahir as 'tanggal', 'sibling' as 'of' from `user` where email <> '".$target."' and tanggal_lahir >= '".$myself[0]["tanggal_lahir"]."' and 
						email <> '".$target."' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$parentKey."')
						";
			}
			else{
				$q = "	select *, tanggal_lahir as 'tanggal', 'child' as 'of' from `user` 
						where email <> '".$target."' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$masterCouple."')
						union
						select *, tanggal_lahir as 'tanggal', 'sibling' as 'of' from `user` where email <> '".$target."' and tanggal_lahir >= '".$myself[0]["tanggal_lahir"]."' and 
						email <> '".$target."' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$parentKey."')
						";
			...
			if(count($parent) > 0){
				if($parent[0]["tanggal_meninggal"] != "0000-00-00" && $parent[0]["hidup"] != "1"){
					$q .= " union select *, tanggal_meninggal as 'tanggal', 'father_died' as 'of' from `user` where email = '".$parent[0]["email"]."' and tanggal_meninggal <> '0000-00-00' ";
				}
			}
			if(count($mother) > 0){
				if($mother[0]["tanggal_meninggal"] != "0000-00-00" && $mother[0]["hidup"] != "1"){
					$q .= " union select *, tanggal_meninggal as 'tanggal', 'mother_died' as 'of' from `user` where email = '".$mother[0]["email"]."' and tanggal_meninggal <> '0000-00-00' ";
				}
			}
			if($myself[0]["tanggal_meninggal"] != "0000-00-00" && $myself[0]["hidup"] != "1"){
				$q .= " union select *, tanggal_meninggal as 'tanggal', 'me_died' as 'of' from `user` where email = '".$target."' ";
			}
			
			$q .= " order by tanggal ";
			//echo $q;
			$lifestory  = $this->Modul_Model->read($q);
		}
		...
		$result = array("myself" => $myself, "parent" => $parent, "mother" => $mother, "couple" => $couple, "child" => $child, "sibling" => $sibling, "lifestory" => $lifestory, 
	...
	public function person(){		
		...
		$masterGroup	= $this->Modul_Model->read("select `group` from person where `key` = '".$user."' and `index` = 0 ")[0]["group"];
		//$masterGroup	= $this->input->post("masterGroup");
		...
		if($ACTION_TYPE == 11){
			$profile		= $this->Modul_Model->read("select * from person where `key` = '".$key."' or `couple` = '".$key."' and `group` = '".$masterGroup."'  ");
		...
		if($ACTION_TYPE == 12){			
			...
			if(count($profile) > 0){
				...
				if($this->Modul_Model->execute($query)){
					$query = "update person set `couple` = '".$email."' where `key` = '".$key."' and `group` = '".$masterGroup."' ";
					$this->Modul_Model->execute($query);
		...
		if($ACTION_TYPE == 13 || $ACTION_TYPE == 14){
			...
			if(count($profile) > 0){
				...
				if($this->Modul_Model->execute($query)){
					...
					$group	= $profile[0]["group"]; $group	= $masterGroup;
		...
		if($ACTION_TYPE == 15){
			$profile		= $this->Modul_Model->read("select * from person where `key` = '".$key."' and `group` = '".$masterGroup."'  ");
			
			if(count($profile) > 0){
				$email = $this->buildCustomID($user);
				$query = "insert into `user` (email, first_name, last_name, marga, jenkel, hidup, tanggal_lahir, tempat_lahir, tanggal_meninggal, tempat_meninggal, alamat, kota, status, user_saved, date_saved) 
								values ('".$email."', '".$first_name."', '".$last_name."', '".$marga."', '".$jenkel."', '".$hidup."', '".$tanggal_lahir."', '".$tempat_lahir."', 
								'".$tanggal_meninggal."', '".$tempat_meninggal."', '".$alamat."', '".$kota."', '2', '".$user."', now())";
				...
				if($this->Modul_Model->execute($query)){
					...
					$queryCouple = "insert into `user` (email, first_name, jenkel, status, user_saved, date_saved) 
									values ('".$emailCouple."', '', '".$jenkelCouple."', '2', '".$user."', now())";
					...
					$query = "update person set parent = '".$email."' where `key` = '".$profile[0]["key"]."' and `group` = '".$masterGroup."'  ";
					$this->Modul_Model->execute($query);
		...
		if($ACTION_TYPE == 41){
			$profile		= $this->Modul_Model->read("select * from person where (`key` = '".$key."' or `couple` = '".$key."') and `group` = '".$masterGroup."'  ");
			$userTarget		= $this->Modul_Model->read("select * from `user` where `email` = '".$key."' ");
			
			$targetIsUser 	= false;
			if(count($userTarget) > 0){
				if($userTarget[0]["is_user"] == 1){
					$targetIsUser = true;
				}
				else{
					$targetUsedCount = $this->Modul_Model->read("select count(*) as 'total' 
								from person where (`key` = '".$key."' or `couple` = '".$key."') ")[0]["total"];
					if($targetUsedCount > 1){
						$targetIsUser = true;
					}
				}
			}
			...
			$data	= $this->Modul_Model->read("select * from `user` where email = '".$key."'")[0];
			$person	= $this->Modul_Model->read("select * from person where `key` = '".$profile[0]["key"]."' and `group` = '".$masterGroup."'")[0];
			if($targetIsUser == false){
				if(count($profile) > 0){
					
					$isCoupleTarget = false;
					if($profile[0]["couple"] == $key ||
						$profile[0]["couple"] != "-"){ /* Is Deleted Couple -  */
						$isCoupleTarget = true;	
					}
					
					$isTargetHasChild = 0;
					if($isCoupleTarget){
						$isTargetHasChild = $this->Modul_Model->read("select count(*) as 'count' 
							from person where `parent` = '".$profile[0]["key"]."' and `group` = '".$masterGroup."' ")[0]["count"];
					}
					
					
					if($isTargetHasChild > 0 ||
						$profile[0]["couple"] != "-"){
						$query = "update `user` set first_name = '', user_saved = '".$user."', date_saved = now() where email = '".$key."' ";
					
						if($this->Modul_Model->execute($query)){
							
							$data	= $this->Modul_Model->read("select * from user where email = '".$key."'")[0];
							$person	= $this->Modul_Model->read("select * from person where `key` = '".$profile[0]["key"]."'")[0];
							$r 		= 1; $msg = "Delete Person Success ";
							
							$files = ['./assets/images/person/' . $key . ".png", './assets/images/person/small/' . $key . ".png"];
							$this->removeFile($files);
						}
					}
					else{
						$query = "delete from person where `key` = '".$profile[0]["key"]."' and `group` = '".$masterGroup."'  ";
						if($this->Modul_Model->execute($query)){
							$query = "delete from `user` where `email` = '".$key."' ";
							if($this->Modul_Model->execute($query)){
								$r 		= $ACTION_TYPE; $msg = "Delete Person Success ";
		...
		private static final int REMOVE_PERSON = 41; String ADD_PARENT_TEXT = "Add Parent ( boy )";*/
	}
	...
	public function searchRelative(){
		...
		$myProfile = $this->Modul_Model->read("select * from person where `key` = '".$user."'")[0];
						
		$query1 = "select u.*, 
						ifnull((select `status` from `relative` where `relative` = u.email and `user` = '".$user."' and `status` <> 40 limit 0, 1 ), -1) as 'relative_status',
						ifnull((select count(*) from `person` where (`key` = u.email or `couple` = u.email) and `group` = '".$myProfile["group"]."' ), 0) as 'my_relative',
						(select `key` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'master',
						(select `group` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'group'
						...
						and (u.email in (select relative from relative where `user` = '".$user."' and `status` <> 40) 
							or u.email in (select relative from relative where `user` in ( select relative from relative where `user` = '".$user."' and `status` <> 40)) )  ";
						
		$query2 = "select u.*, ifnull((select `status` from `relative` where `relative` = u.email and `user` = '".$user."' and `status` <> 40 limit 0, 1 ), -1) as 'relative_status' ,
						ifnull((select count(*) from `person` where (`key` = u.email or `couple` = u.email) and `group` = '".$myProfile["group"]."' ), 0) as 'my_relative',
						(select `key` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'master',
						(select `group` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'group'
						from `user` u where u.email <>  '".$user."' and  ( lower(u.first_name) like '%".$keyword."%' or lower(u.last_name) like '%".$keyword."%' or lower(u.marga) like '%".$keyword."%' )
		...
		$query3 = "select u.*, ifnull((select `status` from `relative` where `relative` = u.email and `user` = '".$user."' and `status` <> 40 limit 0, 1 ), -1) as 'relative_status' ,
						ifnull((select count(*) from `person` where (`key` = u.email or `couple` = u.email) and `group` = '".$myProfile["group"]."' ), 0) as 'my_relative',
						(select `key` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'master',
						(select `group` from person where `index` = 0 and `group` = (select `group` from person where `key` = u.email or `couple` = u.email limit 0, 1)) as 'group'
						from `user` u where u.email <>  '".$user."' and  ( lower(u.first_name) like '%".$keyword."%' or 
						lower(u.last_name) like '%".$keyword."%' or lower(u.marga) like '%".$keyword."%' )  ";

		
		$person1 = $this->Modul_Model->read($query1 . " order by u.first_name"); 
		if(count($person1) > 0){
		...
		if(count($data) <= 10){
			...
			$person2 = $this->Modul_Model->read($query2 . " order by u.first_name"); 
			if(count($person2) > 0){
		...
		if(count($data) <= 10){
			...
			//echo $query3 . '<br /><br /><br />';
			$person3 = $this->Modul_Model->read($query3 . " order by u.first_name"); 
			if(count($person3) > 0){
	...
	public function addFact(){
		...
		$query = "";
		if($type == "birth"){
			$query = "update `user` set tanggal_lahir = '".$date."', tempat_lahir = '".$location."', user_saved = '".$user."', date_saved = now() where email = '".$target."' ";
		}
		else if($type == "marriage"){
			$query = "update `user` set tanggal_menikah = '".$date."', tempat_menikah = '".$location."', user_saved = '".$user."', date_saved = now() where email = '".$target."' ";
		}
		else if($type == "death"){
			$query = "update `user` set tanggal_meninggal = '".$date."', tempat_meninggal = '".$location."', hidup = 0, user_saved = '".$user."', date_saved = now() where email = '".$target."' ";
		}
		
		$result		= array();
		if($this->Modul_Model->execute($query)){			
			$data	= $this->Modul_Model->read("select * from user where email = '".$target."'")[0];
			$person	= $this->Modul_Model->read("select * from person where `key` = '".$target."'")[0];
			
			$result = array("result" => 1, "data" => $data, "person" => $person, "msg" => "Add Fact Success");
		}
		else{
			$result = array("result" => 0, "msg" => "Add Fact Failed");
		}
		echo json_encode($result);
	}
	...
	public function getPersonDetail2(){
		//$user = $this->input->post("user");
		$user = 'hvezaskar@gmail.com';
		
		$target = $this->input->post("target");
		//$target = $this->input->get("target");
		$target = "hvezaskar@gmail.com_2487801";
		//$target = "uqhy@gmail.com_6395711";
		//$target = "uqhy@gmail.com_9837295";
		...
		$masterCouple = $this->Modul_Model->read("select `key` from person where `couple` = '".$target."'");
		//echo json_encode($masterCouple);
		if(count($masterCouple) > 0){
			$isCouple = true;
			$masterCouple = $masterCouple[0]["key"];
		...
		if(!$isCouple){
			...
			$parentKey = "";
			$group		= $this->Modul_Model->read("select `group` from person where `key` = '".$masterCouple."'")[0]["group"];
			$child		= $this->Modul_Model->read("select * from `user` where email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$masterCouple."') order by tanggal_lahir ");
		}
		...
		if($parent != "-"){
			if(!$isCouple){
				$parent		= $this->Modul_Model->read("select * from `user` where email = '".$parentKey."' ");	
				$mother		= $this->Modul_Model->read("select * from `user` where email = (select couple from person where `key` = '".$parentKey."' limit 0, 1) ");	
				$sibling	= $this->Modul_Model->read("select * from `user` where email <> '".$target."' and email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$parentKey."') order by tanggal_lahir ");	
				$couple		= $this->Modul_Model->read("select * from `user` where email in (select `couple` from person where `group` = '".$group."' and `key`  = '".$target."') order by tanggal_lahir ");	
			}
			else{
			...
			if(!$isCouple){
				...
				$q = "	select *, tanggal_lahir as 'tanggal', 'child' as 'of' from `user` 
						where email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$masterCouple."')
						union
						select *, tanggal_lahir as 'tanggal', 'sibling' as 'of' from `user` where email <> '".$target."' and tanggal_lahir >= '".$myself[0]["tanggal_lahir"]."' and 
						email in (select `key` from person where `group` = '".$group."' and `parent`  = '".$parentKey."')
						";
	...
	public function relation2(){
		...
		$person = $this->Modul_Model->read("select p.*, (select count(*) from person pp where pp.`parent` = p.`key`) as 'childCount' from person p where p.`group` = '".$group."' order by p.`index`");
		$user	= $this->Modul_Model->read("select * from `user` where email in (select `key` from person where `group` = '".$group."' order by `index`) or email in (select `couple` from person where `group` = '".$group."') ");
	...
	public function seemRelative2(){
		...
		$q = " select * from `user` where email in ( select `key` from person where `group` = '".$me["group"]."' )";
		
		$myRelative	= $this->Modul_Model->read($q);
